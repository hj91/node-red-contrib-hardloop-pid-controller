...
[![simple-pid-controller](https://img.shields.io/badge/simple--pid--controller-v2.0.1-green)](https://www.npmjs.com/package/simple-pid-controller)
...
2. **PID runs in engineering units** -- The scaling happens inside the node, not in the flow. The engineer configures AI count range and EU range once in the node editor. The PID always operates in physically meaningful values (degC, bar, RPM) regardless of the card resolution.
...
|---|---|---|
| Temperature control (oven, tank, reactor) | 500ms - 2000ms | Classic PID application, ideal fit |
| Liquid level control | 500ms - 2000ms | Slow dynamics, excellent fit |
| Pressure control (vessel, pipeline) | 200ms - 1000ms | Depends on vessel volume |
| Flow control (liquid) | 100ms - 500ms | Acceptable at lower end of range |
| Humidity control | 1000ms - 5000ms | Very slow, ideal fit |
| pH / conductivity control | 500ms - 2000ms | Chemical dosing processes |
...
| **AI/AO independent config** | N/A | Yes -- separate count ranges |
...
| **Fault field in output** | No | Yes -- `fault` boolean |
...
Both nodes use the same `simple-pid-controller` v2.0.1 core, so all v2.0.1 features (anti-windup, output clamping, derivative on measurement, bumpless transfer, runtime gain tuning, reset, EventEmitter) are present in both.
...
(pressure, temp,           samples signal             msg.payload = 2731  (raw count)
 flow, level)              stores integer count   ->
                           in hardware register
                                                           |
                                                           v  countToEU()
                                                       2731 counts -> 68.27 degC
                                                           |
                                                           v
                                                       PID controller (EU domain)
                                                       SV=75 degC, PV=68.27 degC
                                                       output = 4.2 degC equivalent
                                                           |
                                                           v  euToAOCount()
                                                       4.2 degC -> 1680 counts
                                                           |
                           AO card DAC            <-       v
                           count -> mA/V               msg.payload.AO_count = 1680
                           drives valve/VFD/heater     (write to AO register via
                                                        Modbus TCP / SLMP / OPC UA)
```
...
2. Configure it:
   ...
   - Set EU range (e.g. 0-100 for 0-100 degC sensor).
   - Set watchdog timeout (recommend 3x your loop dt in ms).
...
| AI Count Max | `4000` | Maximum count. Common values: 4000, 8000, 16000 -- check your card datasheet |
...
| Field | Default | Description |
|---|---|---|
| EU Min | `0` | Engineering unit value at count_min (e.g. 0 degC, 0 bar) |
| EU Max | `100` | Engineering unit value at count_max (e.g. 100 degC, 10 bar) |
| Signal Type | `4-20mA` | Physical signal standard -- affects Signal output field and documentation only. Count scaling is purely linear. |
...
| Deadband (EU) | `0` | Suppress output when abs(error) <= deadband. Set 0 to disable |
| Settled Tolerance (EU) | `0` | Stop loop when abs(error) <= tolerance. Set 0 to disable |
| Derivative Filter alpha | `1.0` | Low-pass filter on D term. 1.0 = no filter. 0.3 = moderate. 0.1 = heavy. |

### Watchdog and Safe State

| Field | Default | Description |
|---|---|---|
| Watchdog (ms) | `3000` | Max time between PV updates before fault. Recommend 3x loop dt |
...
| `auto` | `true` / `false` | `true` -- start loop. `false` -- stop loop, switch to manual mode |
| `gains` | `{ k_p, k_i, k_d }` | Update PID gains at runtime without restarting |
| `mode` | `"auto"` or `"manual"` | Switch mode. `manual->auto` performs bumpless transfer |
...
**Recommended watchdog timeout:** 3x your loop `dt`. For a 1-second loop, set watchdog to 3000ms.
...
|---|---|---|
| 0 - 4000 | ~12-bit | Mitsubishi Q64AD, FX3U-4AD, many Siemens S7-300 SM331 |
| 0 - 8000 | ~13-bit | Mitsubishi iQ-R R60AD, some OMRON CJ1W |
| 0 - 16000 | ~14-bit | Mitsubishi Q68AD, R60ADI, Yokogawa high-res cards |
| 0 - 27648 | 15-bit | Siemens S7-1200 / S7-1500 standard AI modules |
| 0 - 32767 | 15-bit | Generic 15-bit ADC, some Beckhoff EL3xxx |
| 0 - 65535 | 16-bit | High-resolution process cards, Beckhoff EL3174 |
...
The `Derivative Filter alpha` parameter applies a first-order low-pass (exponential moving average) filter:

```
D_filtered(t) = alpha x D_raw(t) + (1 - alpha) x D_filtered(t-1)
```
...
- PID computation via `simple-pid-controller` v2.0.1 in EU domain
...
- Derivative low-pass filter (configurable alpha) for ADC quantisation noise mitigation
- `scan_ms` field for loop jitter monitoring
- `fault`, `watchdog_ok`, `mode` fields in every output message
- All `simple-pid-controller` v2.0.1 features: anti-windup, output clamping, deadband, bumpless transfer, runtime gain tuning, reset, settled tolerance
